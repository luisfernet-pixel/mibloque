    ...
    <main className="landing-sell theme-shell-dark relative min-h-screen overflow-hidden">
      ...
      <section className="mx-auto grid max-w-6xl gap-8 px-4 pb-10 pt-6 sm:px-6 lg:grid-cols-[1.08fr_0.92fr] lg:px-8 lg:pt-12">
        <div>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            {coreBenefits.map((item, index) => (
              <div
                key={item}
                className="landing-card landing-chip-card rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200"
        ...
        <div className="landing-ideal-panel rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,_rgba(7,20,38,0.92),_rgba(17,32,56,0.88))] p-6 shadow-[0_28px_80px_rgba(2,6,23,0.42)]">
          ...
          <h2 className="mt-3 text-xl font-bold text-white">Edificios, condominios y conjuntos residenciales</h2>
          ...
          <div className="mt-6 grid gap-3">
            {modules.map((item, index) => (
              <article
                key={item.title}
                className="landing-card landing-service-card rounded-[1.6rem] border border-white/10 bg-white/5 p-4 shadow-[0_18px_45px_rgba(2,6,23,0.2)]"
                style={
                  {
