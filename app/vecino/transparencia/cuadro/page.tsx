...
export default async function VecinoCuadroTransparenciaPage({
  searchParams,
}: {
  ...
  for (const vecino of vecinos) {
    ...
    vecinosPorDepto.set(deptoId, String(vecino.nombre || "Sin asignar"));
  }
  ...
  for (const cuota of cuotas) {
    ...
    const current = String(cuotaKey.get(key) || "");
  ...
  return (
    <main className="space-y-6">
      ...
      <CuadroVitrina
        ...
        subtitle="Matriz general de pagos por departamento y mes."
        year={anio}
        rows={rows}
        highlightDepartamentoId={usuario.perfil.departamento_id}
      />
    </main>
  );
}
